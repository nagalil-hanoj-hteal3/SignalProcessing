...
%time  = (1:npnts)/srate; % leads to 2 with "time(1, end)" command
time  = (0:npnts-1)/srate; %<- 1.992
...
% NOTE: multiplying the coefficient with randn makes the noise difficult to distinguish from signal
signal = signal + 20*randn(size(signal));
...
% adds the red circles within each change of behavior
subplot(211), hold on
...
% search volume begins to decrease then increase over time (inconsistent)
% pattern stays the same but adjusts the y-axis
figure(2), clf
subplot(211)
plot(searchdata,'ko-','markerfacecolor','m','linew',1)
xlabel('Time (weeks)'), ylabel('Search volume')

% Result provided with peaks from all over the frequency
subplot(212)
...
% search volume begins to decrease over time
%figure(2), clf
%subplot(211)
%plot(searchdata,'ko-','markerfacecolor','m','linew',1)
%xlabel('Time (weeks)'), ylabel('Search volume')

% Result is very skewed from positive to nearly zero
%subplot(212)
%plot(hz,searchpow,'ms-','markerfacecolor','k','linew',1)
%xlabel('Frequency (norm.)'), ylabel('Search power')
%set(gca,'xlim',[0 12])
